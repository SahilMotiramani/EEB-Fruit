...
export default function Cart() {
    ...
    const calculateDiscountedPrice = (price) => {
        return isCouponApplied ? price - (price * discount) : price;
    };

    // Calculate the total after discount is applied
    const discountedTotal = items.reduce((acc, item) => {
        const price = parseFloat(item.price); // Ensure price is a number
        return acc + (calculateDiscountedPrice(price) * item.quantity);
    }, 0);
            ...
            {items.length === 0 ? (
                ...
                <div className="mx-10 lg:mx-24">
                                ...
                                {items.map(item => {
                                    ...
                                    return (
                                        <tr key={item.id} className="border-b">
                                            ...
                                            <td className="px-6 py-4">
                                                <div>
                                                    <span style={{ textDecoration: isCouponApplied ? 'line-through' : 'none' }}>₹{originalPrice.toFixed(2)}</span>
                                                    {isCouponApplied && (
                                                        <span className="text-green-500"> (Discounted: ₹{discountedPrice.toFixed(2)})</span>
                                                    )}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                    ...
                    <div className="mt-16 flex flex-col sm:flex-row ">
                        <input 
                            className="w-44 py-4 px-2 border-b-2 border-gray-200 rounded-lg" 
                            type="text" 
                        ...
                        <button 
                            ...
                            className="w-fit px-6 py-4 font-semibold sm:mx-6 my-6 sm:my-0 border border-orange-400 rounded-full duration-500 coupon"
                        >
                            Apply Coupon
                        </button>
                    </div>
                        ...
                        <div className="mt-12 bg-gray-100 p-6 rounded-lg justify-end w-full lg:w-6/12">
                            <div style={{ color: '#45595b' }}>
                                ...
                                <div className="flex justify-between my-4">
                                    ...
                                    <span>₹{(subtotal - (subtotal * discount)).toFixed(2)}</span>
                                </div>
                                ...
                                <div className="flex justify-between my-4">
                                    ...
                                    <span>₹{(discountedTotal + 3).toFixed(2)}</span>
                                </div>
                                <hr />
                            </div>
                            <button 
                                onClick={handleCheckout} // Call handleCheckout to navigate
                                className="w-fit px-6 py-4 font-semibold mt-4 border border-orange-400 rounded-full uppercase duration-500 checkout"
                            >
                                Proceed Checkout
                            </button>
                        </div>
